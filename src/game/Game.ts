...
import { COLORS, PLAYER, toRoman } from "./config";
...
export class Game implements GameEvents {
  ...
  onEnemyKilled(enemy: EnemyTarget, distance: number, melee: boolean): void {
    ...
    let bonus = 0;
    let label = "";
    if (melee) { bonus += 20; label = "EXECUTION"; }
    else if (this.player.isSliding) { bonus += 15; label = "SLIDE KILL"; }
    else if (this.player.isAirborne) { bonus += 20; label = "AIR KILL"; }
    else if (now - this.lastKillAt < 800) { bonus += 30; label = "MULTI-KILL"; }
    const base = enemy.kind === "stalker" ? 34 : 24;
    this.marks += base + bonus;
    if (distance < 3.4 || melee) this.blood = Math.min(PLAYER.maxBlood, this.blood + (melee ? 14 : 7));
    this.lastKillAt = now;
  ...
  private update(): void {
    ...
    if (this.phase === "playing" || this.phase === "intermission") {
      ...
      this.moon.update(delta, this.player.position);
      this.enemies.update(delta, this.player.position, this.moon.state === "combat");
      this.updateShopPrompt();
      const intendedPhase = this.moon.state === "intermission" ? "intermission" : "playing";
      if (intendedPhase !== this.playPhase) {
        this.playPhase = intendedPhase;
        this.phase = intendedPhase;
      }
    } else {
      this.player.update(delta);
      this.weapons.update(delta);
    ...
    if (this.phase === "dying") {
      ...
      if (this.scene.fog instanceof THREE.FogExp2) {
        this.scene.fog.density = Math.min(0.08, 0.018 + (2.6 - this.deathTimer) * 0.022);
      }
      if (this.deathTimer <= 0) this.showTally();
    }
    this.renderer.toneMappingExposure += (0.95 - this.renderer.toneMappingExposure) * (1 - Math.exp(-8 * delta));
    this.renderHud();
  ...
  private showTally(): void {
    this.setPhase("tally");
    this.ui.showTally(this.save.data.attempts, this.moon.currentMoon, this.kills);
    this.ui.setAttempt(this.save.data.attempts);
  }
  ...
  private updateShopPrompt(): void {
    this.isNearShop = this.environment.distanceToShop(this.player.position) < 5.4;
    if (!this.isNearShop) {
  ...
  private useShop(): void {
    if (!this.isNearShop || !["playing", "intermission"].includes(this.phase)) return;
    if (!this.weapons.hasShotgun) {
      if (this.marks < 450) { this.ui.reward("NOT ENOUGH MARKS"); return; }
      this.marks -= 450;
      this.weapons.purchaseShotgun();
      this.ui.announce("WIDOWMAKER 12G", "PURCHASED // PRESS 2 TO EQUIP", 2.2);
      return;
    }
    if (this.marks < 120) { this.ui.reward("NOT ENOUGH MARKS"); return; }
    if (!this.weapons.refillAmmo()) { this.ui.reward("AMMO ALREADY FULL"); return; }
    this.marks -= 120;
  ...
  private buildLighting(): void {
    ...
    for (const [x, z] of [[-21, -17], [17, -17], [-20, 17], [20, 17]]) {
      const fluorescent = new THREE.PointLight(COLORS.mallCyan, 12, 16, 2);
  ...
  private bindActions(): void {
    ...
    this.canvas.addEventListener("click", () => {
      ...
      else if (["playing", "intermission"].includes(this.phase) && document.pointerLockElement == null) this.captureMouse();
    });
    document.addEventListener("pointerlockchange", () => {
      ...
      if (!locked && ["playing", "intermission"].includes(this.phase)) this.setPhase("paused");
      else if (locked && this.phase === "paused") this.setPhase(this.playPhase);
    });
    window.addEventListener("keydown", (event) => {
      if (event.code === "KeyE" && !event.repeat) this.useShop();
      if (event.code === "Escape" && !event.repeat && ["playing", "intermission"].includes(this.phase)) {
  ...
  private get pointerLockAvailable(): boolean {
    return typeof this.canvas.requestPointerLock === "function" && "pointerLockElement" in document;
  }
  ...
  private captureMouse(): void {
    if (this.pointerLockAvailable) {
      try {
        ...
        if (request instanceof Promise) void request.catch(() => this.activatePreviewLook());
        window.setTimeout(() => {
          if (document.pointerLockElement !== this.canvas && ["playing", "intermission", "paused"].includes(this.phase)) {
  ...
  private activatePreviewLook(): void {
    ...
    this.ui.announce("PREVIEW LOOK ACTIVE", "MOVE TO LOOK // HOLD AT EDGE FOR 360 TURN // ESC TO PAUSE", 3.4);
  }
